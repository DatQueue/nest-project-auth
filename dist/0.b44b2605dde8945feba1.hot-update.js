...
let AuthService = class AuthService {
    ...
    async refresh(refreshToken) {
        const user = await this.userService.getUserByRefreshToken(refreshToken.refresh_token);
        if (!user) {
            throw new common_1.NotFoundException('User not foundsssssssssssssss');
        }
        const isValidRefreshToken = await this.userService.getUserIfRefreshTokenMatches(refreshToken.refresh_token, user.id);
    ...
    async logout(req) {
        ...
        if (!refreshToken) {
            throw new common_1.BadRequestException('Refresh Token not found in cookies!');
        }
...
/******/ (() => {
/******/ 	__webpack_require__.h = () => ("8cccdaea4e100f131334")
/******/ })();
/******/ 
/******/ }
;